...
function vr11632_cols(string $table):array{
 $rows=gdb_all(
...
$key=(string)($_GET['key']??'');
if(!hash_equals(vr11632_key(),$key)){
...
try{
 $cols=vr11632_cols('local_ai_tasks');
 $sets=["status='queued'"];
 ...
 echo json_encode([
  'ok'=>true,
  'version'=>'V116.3.2 Voice Task Reset',
  'tasks_requeued'=>$stmt->rowCount(),
  'next'=>'Start the V116.3.2 voice runtime.',
  'time'=>date('c')
