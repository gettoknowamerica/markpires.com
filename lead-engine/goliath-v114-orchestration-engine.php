...
$V112_RESPONSE=['ok'=>false,'version'=>'V114.0 Constitutional Orchestration Engine'];
...
if((int)($lock['acquired']??0)!==1){echo json_encode(['ok'=>true,'version'=>'V114.0 Constitutional Orchestration Engine','status'=>'skipped_overlap']);exit;}
...
try{
 ...
 p112_all("UPDATE goliath_v112_stages SET status='ready',local_task_id=NULL,last_error='V114 recovered stale stage',updated_at=NOW() WHERE status IN ('queued_local','working') AND updated_at<DATE_SUB(NOW(),INTERVAL 45 MINUTE)");
 ...
 if($ready&&$taskCols){
   ...
   $taskId=g114_create_local_task(
     (string)$ready['executive_key'],
     'goliath_v112_stage',
     'V114 '.$ready['executive_key'].' — '.$ready['title'],
     $prompt,
     500,
     ['mission_id'=>$ready['mission_id'],'stage_id'=>$ready['id'],'stage_no'=>$ready['stage_no'],'stage_key'=>$ready['stage_key'],'originator_key'=>$ready['originator_key']]
   );
   gdb_update('goliath_v112_stages',[
 ...
 echo json_encode([
  'ok'=>true,'version'=>'V114.0 Constitutional Orchestration Engine','status'=>'complete',
 ...
 echo json_encode([
  'ok'=>false,
  'version'=>'V114.0 Constitutional Orchestration Engine',
