...
try{
 ...
 foreach($execs as $e){if(gdb_one("SELECT id FROM executive_council_reports WHERE council_date=? AND executive_key=? LIMIT 1",[$date,$e]))continue;$inits=gdb_all("SELECT title, reason FROM executive_initiatives WHERE executive_key=? AND status='recommended' ORDER BY priority DESC,id DESC LIMIT 3",[$e])?:[];$titles=array_map(fn($x)=>$x['title'],$inits);ins('executive_council_reports',['report_uid'=>uid('council'),'council_date'=>$date,'executive_key'=>$e,'completed_today'=>'V101.1 organization pulse active.','learned_today'=>'Idle time is unacceptable. I must create or advance missions without waiting for Mark.','recommends_next'=>count($titles)?implode("\n",$titles):'Create one high-impact mission for tomorrow.','needs_help'=>'Goliath should assign shared missions when collaboration improves the outcome.','proposed_missions_json'=>json_encode($inits,JSON_UNESCAPED_SLASHES),'created_at'=>gdb_now()]);$created[]=$e;}
 echo json_encode(['ok'=>true,'version'=>'V101.1 Executive Council Pulse','created_reports'=>$created,'time'=>date('c')],JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES);
}catch(Throwable $e){echo json_encode(['ok'=>false,'version'=>'V101.1 Executive Council Pulse','error'=>$e->getMessage(),'file'=>$e->getFile(),'line'=>$e->getLine()],JSON_PRETTY_PRINT);}
?>