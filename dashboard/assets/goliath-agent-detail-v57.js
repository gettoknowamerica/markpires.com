(function(){
  ...
  function renderList(){
    ...
    $('g54List').innerHTML=arr.length?arr.map(it=>`<div class="g54WorkItem ${selected&&selected.id===it.id&&selected.source===it.source?'active':''}" data-index="${data.items.indexOf(it)}"><strong>${esc(titleFrom(it))}</strong><small>${esc(it.subtitle||it.summary||'Open work')}</small><div class="g54Meta"><span class="g54Pill">${esc(it.kind)}</span><span class="g54Pill">${esc(it.status)}</span>${it.score?`<span class="g54Pill">Score ${esc(it.score)}</span>`:''}<span class="g54Pill">${esc(it.source)}</span></div></div>`).join(''):'<div class="g54Skeleton">No items in this view yet. When the Executive completes work, it appears here.</div>';
  ...
  function renderStructured(it){
    const p=payload(it), wd=workData(it), text=outputText(it);
    let html='';
    if(isObj(wd)){
      const summary=val(wd,['summary','status','recommendation'],'');
      if(summary) html+=`<div class="g54Summary">${cleanMarkdown(summary)}</div>`;
    ...
    if(!html && text) html=cleanMarkdown(text);
  ...
  function renderContent(it){
    const p=payload(it), wd=workData(it), kind=(it.kind||'').toLowerCase();
    ...
    if(isLeadKind(kind)){
      return `<div class="g54Doc"><h1>${esc(titleFrom(it))}</h1><p>${esc(unescapeText(it.subtitle||''))}</p>${renderActions(p,wd)}<div class="g54LeadCard">${renderFields(p,wd)}</div><h3>Work Summary</h3>${renderStructured(it)}${renderRawDetails(p,wd)}</div>`;
    }
    return `<div class="g54Doc"><h1>${esc(titleFrom(it))}</h1><p>${esc(unescapeText(it.subtitle||it.summary||''))}</p>${renderActions(p,wd)}<h3>Work Product</h3>${renderStructured(it)}${renderRawDetails(p,wd)}</div>`;
  }
  ...
  async function sendIntercom(){
    const box=$('g54Prompt'), ack=$('g57IntercomAck');
    const msg=(box&&box.value||'').trim();
    if(!msg){ if(ack) ack.innerHTML='<div class="g57AckCard"><strong>Nothing sent yet.</strong><span>Give '+esc(agent)+' a direction first.</span></div>'; return; }
    if(ack) ack.innerHTML='<div class="g57AckCard"><strong>Sending to '+esc(agent)+'...</strong><div class="g57ProgressWrap"><div class="g57ProgressTrack"><div class="g57ProgressFill" style="width:3%"></div></div><div class="g57ProgressMeta"><span>Founder direction captured</span><span>3%</span></div></div></div>';
    try{
      const res=await fetch('/dashboard/api/executive-intercom.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({executive:agent,message:msg,source_item_id:selected?(selected.id||''):''})});
      const data=await res.json();
      if(!data.success) throw new Error(data.error||'Intercom failed');
      const pct=(data.progress&&data.progress.percent)||5;
      if(ack) ack.innerHTML='<div class="g57AckCard"><strong>'+esc(agent)+' acknowledged.</strong><span>'+esc(data.acknowledgement||'Understood, Mark. I am on it.')+'</span><div class="g57ProgressWrap"><div class="g57ProgressTrack"><div class="g57ProgressFill" style="width:'+esc(pct)+'%"></div></div><div class="g57ProgressMeta"><span>'+esc((data.progress&&data.progress.phase)||'Queued')+'</span><span>'+esc(pct)+'%</span></div></div></div>';
      box.value='';
      setTimeout(load,900);
    }catch(e){ if(ack) ack.innerHTML='<div class="g57AckCard"><strong>Intercom needs attention.</strong><span>'+esc(e.message||e)+'</span></div>'; }
  }
  window.g54SendIntercom=sendIntercom;
  window.g54ClearPrompt=function(){ if($('g54Prompt')) $('g54Prompt').value=''; if($('g57IntercomAck')) $('g57IntercomAck').innerHTML=''; }
  window.g54CopyPrompt=function(){ sendIntercom(); }
  load();
})();
